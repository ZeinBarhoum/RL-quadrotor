...
def train():
    ...
    n_actions = env_no_gui_train.action_space.shape[-1]
    ...
    model = DDPG("MlpPolicy", train_env, verbose= 1, policy_kwargs= dict(activation_fn=torch.nn.ReLU,net_arch=[512, 512, 256, 128]), action_noise=action_noise)
    
    model.learn(total_timesteps=1e5, callback= eval_callback)
    
    model.save("DDPG_TakeOFF")
